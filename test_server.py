...
questions = [{'english': f'question {i}',
              'german': f'Frage {i}'} for i in range(5)]
questions.append({'english': 'question <app_name>',
                  'german': f'Frage <app_name>'})
...
def test_send_question(client):
    ...
    assert request_question(question_id=5,
                            app_name='Telegram') == 'question Telegram'
    assert request_question(question_id=5,
                            app_name='pointcloud',
                            language='german') == 'Frage pointcloud'


def test_receive_answer(client):
    expected_data = {'user_id': 'NULL',
                     'app_name': 'NULL',
                     'question': 'NULL',
                     'answer_text': 'NULL',
    ...
    expected_data.update({'app_name': 'pushrequest',
                          'question': 'You afraid of superintelligence?',
                          'answer_text': 'Oh yes.'})
    response = client.post(
        '/answer', data=b'''{"user_id": "b3f",
